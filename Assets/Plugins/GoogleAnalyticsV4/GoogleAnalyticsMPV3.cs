...
public class GoogleAnalyticsMPV3 {
...
#else
  private const string playerPrefKeyUUID = "ga-UUID";
  private const string glyphs = "abcdefghijklmnopqrstuvwxyz0123456789";
  private const string baseUrl = "https://www.google-analytics.com/collect?v=1";
  ...
  {
    var charAmount = 16; //set those to the minimum and maximum length of your string
    var str = "";
  ...
  private void SendGaHitWithMeasurementProtocol(string url) {
    ...
    HandleWWW(new UnityWebRequest(newUrl));
  }
  ...
  */
  ...
  private string AddRequiredMPParameter(Field parameter, object value) {
    if(!trackingCodeSet){
      ...
      return parameter + "=" + UnityWebRequest.EscapeURL(value.ToString());
    }
  }
  ...
  private string AddRequiredMPParameter(Field parameter, string value) {
    if(!trackingCodeSet){
      ...
      return parameter + "=" + UnityWebRequest.EscapeURL(value);
    }
  }
    ...
    foreach(KeyValuePair<int, string> entry in builder.GetCustomDimensions())
    {
      if (entry.Value != null) {
        url += Fields.CUSTOM_DIMENSION.ToString() + entry.Key + "=" +
               UnityWebRequest.EscapeURL(entry.Value.ToString());
      }
    }

...
public class ResizeTexture2D : Utilities
{
    ...
    void OnGUI()
    {
        EditorGUILayout.LabelField(
            "Click 1 ( or mutil Texture or choose the folder have them if them same size) to Edit");
        EditorGUILayout.LabelField("Change value Width, Height to crop");
        ...
        scrollPositionTexture = EditorGUILayout.BeginScrollView(scrollPositionTexture, GUILayout.Width(position.width),
            ...
            for (int index = 0; index < selectedObjects.Length; index++)
            {
                ...
                if (newTexture2D != null)
                {
                    ...
                    else
                    {
                        ...
                        newTexture = new Texture2D(newTexture2D.width, newTexture2D.height, TextureFormat.RGBA32,
                            false);
    ...
    void DrawTexture(Texture2D texture, int index, int posX, string LabelTitle)
    {
        ...
        EditorGUI.LabelField(new Rect(posX, posLabelY, position.width / 2, 25),
            texture.name + LabelTitle + texture.width + "x" + texture.height);
    ...
    void ChooseOptions(Options options)
    {
        switch (options)
        {
            ...
            case Options.Crop:
                ...
                if (listTexturesOrigin.Count > 0)
                {
                    widthCrop = listTexturesOrigin[0].width < widthCrop ? listTexturesOrigin[0].width :
                        widthCrop <= 0 ? 0 : widthCrop;
                    heightCrop = listTexturesOrigin[0].height < heightCrop ? listTexturesOrigin[0].height :
                        heightCrop <= 0 ? 0 : heightCrop;
                }
    ...
    public class Result
    {
        ...
        public float xRect { get; set; }
        public float yRect { get; set; }
    }
        ...
        string path)
    {
        ...
        {
            return new Result() {texture = texture, xRect = -1, yRect = -1};
        }
        ...
        if (r.width != 0 && r.height != 0)
        {
                ...
                yRect > r.y + texture.height || xRect < 0 || yRect < 0 || r.width < 0 || r.height < 0)
            {
                ...
                return new Result() {texture = result, xRect = xRect, yRect = yRect};
            }
        ...
        return new Result() {texture = result, yRect = yRect, xRect = xRect};
    }
